...
if [ "$STATE" != "running" ]; then
  aws ec2 run-instances  --launch-template LaunchTemplateId=lt-097d20195887becd4 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${component}}]" | jq .
  sleep 15
fi
...
echo "${PUBLIC_IPADDRESS} APP=${component}\n" >>../inventory